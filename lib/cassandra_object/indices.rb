  ...
  module Indices
	...
	index_info
		...
		def write_index_info(col_family, key, sup_col, column, entry_id)
			...
			old_data = self.class.connection.get(ind_ind_cf, self.key.to_s)
			data = (old_data || {}).merge({col_family.to_s => ind_ind_entry})
			self.class.connection.insert(ind_ind_cf, self.key.to_s, data)
		end
